...
LinkGrid.Link = function Link({ title, description, href, customized, viewBox, icon }) {
  return (
    <div className="group relative rounded-xl border border-slate-200 dark:border-slate-800">
      ...
      <div className="relative overflow-hidden rounded-xl p-6">
        {customized ? <CustomIcon icon={icon} className="h-8 w-8" viewBox={viewBox}/> : <Icon icon={icon} className="h-8 w-8" />}
